...
- **API Keys** for OpenRouter and Google CSE (provider-level BYOK for e.g. `openai/o3-pro` is configured at OpenRouter, not in this project's config)
...
1. **OpenRouter** - Sole API gateway for all LLM access (Claude, Grok, GPT, o3-pro, Gemini)
   - Get key at: https://openrouter.ai/
   - Used for: All text generation commands
   - For BYOK access to specific provider models (e.g. `openai/o3-pro`), add the
     provider key at https://openrouter.ai/settings/integrations -- not in
     this project's config.yaml.

2. **Google Custom Search** - For case law lookup
...
LitAssist checks for config.yaml in this order:
1. `LITASSIST_CONFIG` environment variable (if set, uses that path directly)
2. `~/.config/litassist/config.yaml` (the standard location)
...
LitAssist uses chunk-based processing for large documents (50k token chunks) and tiktoken for accurate token counting. If a research file exceeds 128k tokens, a warning is displayed and processing may be truncated or chunked. CLI output includes file, word, and token counts for research/context files.
...
│   ├── draft_*.txt
│   └── strategy_*.txt
├── logs/                 # Audit logs (LLM calls, verification)
│   ├── task_event_*.md
...
**Common Mistake:** Creating `config.yaml` in project directories. The config must be in `~/.config/litassist/` or specified via the `LITASSIST_CONFIG` environment variable.
