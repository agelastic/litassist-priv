...
- Actual per-call cost from OpenRouter. Every LLM request now sets
  ...
  all internal billed SDK calls (tool-fallback follow-up / retries, previously
  uncounted) via `litassist/llm/usage.py:merge_usage`, returned in the usage dict
  and written to the audit log. Replaces the local price-table estimator
  `litassist/llm/cost.py:estimate_call_cost` (removed; it undercounted vs the
  OpenRouter invoice). Capability price fields stay for non-cost uses.
