...
- The case facts input is now optional for `caseplan`, `strategy`, `barbrief` (positional), `brainstorm` (`--facts`), and `draft` (`documents`). When omitted, the new shared `resolve_case_facts_file()` (`litassist/utils/case_facts.py`) picks the latest `case_facts*.txt` in the launch directory and prints which file it chose. Selection is by filename: our timestamps are zero-padded `YYYYMMDD_HHMMSS` and `"."` sorts before `"_"`, so the lexically greatest name is the newest timestamped version (e.g. `case_facts_20260530_101500.txt`); a lone `case_facts.txt` is used as-is. If no `case_facts*.txt` exists, the command fails with a clear message. `brainstorm`'s previous bespoke `case_facts.txt`-only default is replaced by this shared resolver. Commands that produce or do not consume case facts (`extractfacts`, `lookup`, `digest`, `counselnotes`, `verify`, `verify-cove`) are unchanged.
