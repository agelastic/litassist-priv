...
#### June 2026: caseplan groups a run's outputs with a shared run-id
- A caseplan-generated script accumulates outputs into the shared, never-cleaned `outputs/` folder, so a glob like `outputs/brainstorm_creative_*.txt` could match files from a PREVIOUS run. The caseplan system prompt now instructs the model to write the literal placeholder `RUNID` as the first segment of every `--output` value and every `outputs/` glob filename; `plan_generator.py` replaces `RUNID` with one real per-run id (`run<YYYYMMDDHHMMSS>`) before saving the plan and extracting the script. Because the same literal sits on both the producer (`--output RUNID_x`) and consumer (`outputs/RUNID_x_*`) sides, they get the identical real prefix and match by construction - no token parsing. If the model emits no `RUNID`, caseplan warns that the run's outputs are not grouped; a partial-compliance check additionally warns (counting output references against those carrying the run prefix) when only SOME outputs were marked. The run-id is recorded in the plan metadata.
