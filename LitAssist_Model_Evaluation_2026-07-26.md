...
3. **After every `model:` edit:** `litassist refresh` (regenerates `model_capabilities.yaml`; also confirms the OpenRouter id actually exists — do this for the exact Claude 5 / GPT-5.6 ids before writing them into config) and `pytest` (the integrity tests assert config-capabilities consistency).
...
2. **Formatting-police blocks should become structured output.** The clearest wins are the markdown-spacing battles in `lookup.yaml` ("FINAL CRITICAL REMINDER: EVERY header MUST have TWO blank lines before it..."). Every pinned model now supports `response_format`/`structured_outputs` via OpenRouter (see `model_capabilities.yaml`). Requesting a schema and deleting the spacing prose is exactly the "prefer prompt engineering over local parsing / fail fast" direction CLAUDE.md already mandates. Do this per-command as those commands' models are trialled — the swap is when output shape is being re-validated anyway.
...
1. **Now:** add Claude 5 / GPT-5.6 entries to `model_profiles.py` + effort maps (prereq for everything Anthropic/OpenAI); run `litassist refresh` to confirm live ids and pull real OpenRouter pricing into `model_capabilities.yaml`.
...
Note: the container's egress proxy blocked a direct `openrouter.ai/api/v1/models` fetch this session (403), so OpenRouter ids/prices above come from OpenRouter's public model pages via search rather than the catalogue JSON. Run `litassist refresh` to confirm exact ids and live pricing before writing any of them into `model_configs.yaml`.
