...
- [x] **[RESOLVED 17/06/2026] Multi-model cross-check (P1-12) removed after evaluation:** Built and gate-passed, then a reference-grounding probe showed it adds no unique detection value over a plain `verify --reference`. Fabricated-fact detection is source-dependent: with a source supplied, the baseline soundness/reasoning stages catch fabrications (4/4 in testing); without a source both baseline and the cross-check miss them. The cross-check's only margin was redundancy/convergent severity on contradictions, not worth its 3-4x cost (o3-pro panel). Removed on branch `cleanup/remove-crosscheck` (deletes `verify/ensemble.py`, the `--cross-check` flag, the `verification.crosscheck.*` prompts, the four `crosscheck-*` roles, and the local estimator `litassist/llm/cost.py`); the actual OpenRouter per-call cost capture (`litassist/llm/usage.py`) replaces the estimator. P-JUDGE and P2-19: P-JUDGE stays (Phase 2 keystone); P2-19 parked. Evaluation evidence on the published-not-merged `crosscheck-eval-evidence` branch.
