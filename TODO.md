...
4. **Pricing-aware features:** **Actual per-call cost SHIPPED 14/06/2026** — requests set `usage: {include: true}`, so OpenRouter returns the real billed `cost` + a generation `id` in the response; both are captured (`response_parser`), summed across internal billed calls (`litassist/llm/usage.py:merge_usage`), returned in the usage dict, logged, and shown in the `[COST]` banner. This replaced the unreliable local estimator (`litassist/llm/cost.py:estimate_call_cost`, removed — it undercounted vs the OpenRouter invoice, 3-9x on reasoning-heavy o3 calls). Remaining: a per-command `Total tokens/cost` line on every command, a `--budget` flag, and cost-aware model selection (the `input_price_per_mtok`/`output_price_per_mtok` capability fields remain for those non-cost uses).
