...
4. **Pricing-aware features:** `litassist/llm/model_capabilities.yaml` already carries `input_price_per_mtok` and `output_price_per_mtok` per model (refreshable via `litassist refresh`). Consume them for per-call cost logging, a `--budget` flag, cost-aware model selection, and a cost field in the audit log. **First consumer SHIPPED 13/06/2026:** `litassist/llm/cost.py:estimate_call_cost(model, usage)` powers the `[COST]` banner on the P1-12 cross-check. Generalise it to the `--budget`/audit-log/cost-aware-selection uses above (still to be added).
