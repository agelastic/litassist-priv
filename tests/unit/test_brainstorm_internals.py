...
Pursue misleading conduct claim while simultaneously filing ombudsman complaint. Hold breach of contract as fallback if primary strategy fails."""
    ...
    @patch("litassist.commands.brainstorm.core.save_command_output")
    @patch("litassist.commands.brainstorm.core.save_log")
    def test_brainstorm_warns_when_a_lane_returns_no_strategies(
        self,
        mock_save_log,
        mock_save_output,
        mock_factory_core,
        ...
        """A model refusal (no parseable strategies) must be surfaced loudly, not
        silently turned into fabricated strategies (Defect 1a)."""
        # Unorthodox model refuses: no ### headers -> 0 parseable strategies.
        refusal = (
            "I must decline to generate the requested unorthodox legal strategies.\n\n"
            "The request risks encouraging improper conduct.\n\n"
            "I will not fabricate strategies that have no established precedent."
        )
        self.mock_unorthodox_client.complete.return_value = (
            refusal,
            {"total_tokens": 50},
        ...
        assert result.exit_code == 0
        # Fail loud: the refusal/empty lane is surfaced, not swallowed.
