...
class TestLLMClientFactory:
    ...
    def test_command_configs_exist(self):
        ...
        expected_configs = [
            "lookup", "extractfacts", "strategy", "draft"
        ]
    ...
    def test_verification_flags_set_correctly(self):
        ...
        with patch("litassist.llm.CONFIG") as mock_config:
            ...
            for command in strict_commands:
                ...
                assert hasattr(client, "_force_verify")
                assert client._force_verify is True, f"{command} should force verification"
            ...
            for command in lenient_commands:
                client = LLMClientFactory.for_command(command)
                assert hasattr(client, "_force_verify")
                assert client._force_verify is False, f"{command} should not force verification"
    ...
    def test_model_parameter_restrictions(self):
        ...
        with patch("litassist.llm.CONFIG") as mock_config:
            ...
            # Test o1-pro model (strategy)
            strategy_client = LLMClientFactory.for_command("strategy")
            strategy_params = strategy_client.default_params
            
            # o1-pro should only have max_completion_tokens, not max_tokens
            assert "max_completion_tokens" in strategy_params
    ...
    def test_environment_variable_override(self):
        ...
        with patch("litassist.llm.CONFIG") as mock_config:
            ...
            with patch.dict("os.environ", {"LITASSIST_LOOKUP_MODEL": "anthropic/claude-3-5-sonnet-20241022"}):
                client = LLMClientFactory.for_command("lookup")
...
class TestLLMClientFactoryIntegration:
    ...
    def test_factory_creates_working_clients(self):
        ...
        with patch("litassist.llm.CONFIG") as mock_config:
            ...
            commands = ["lookup", "brainstorm", "strategy", "draft", "digest", "extractfacts"]
    ...
    def test_configured_commands_have_specific_models(self):
        ...
        with patch("litassist.llm.CONFIG") as mock_config:
            ...
            assert "o1" in models["strategy"].lower()  # Uses o1-pro for strategy
            assert "o3" in models["draft"].lower()  # Uses o3 for drafting
            assert "claude" in models["extractfacts"].lower()  # Uses Claude for extraction