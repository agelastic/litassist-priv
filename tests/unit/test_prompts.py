...
class TestPromptManager:
    ...
    def test_nonexistent_template_raises_keyerror(self):
        ...
        with pytest.raises(KeyError, match="Template key 'nonexistent.template' not found"):
...
class TestIntegrationWithCommands:
    ...
    def test_extractfacts_command_requires_prompts(self):
        ...
        with patch('litassist.commands.extractfacts.PROMPTS') as mock_prompts:
            mock_prompts.get_format_template.side_effect = KeyError("Template not found")
...
class TestTemplateValidation:
    ...
    def test_australian_law_template_content(self):
        ...
        assert any(word in australian_law.lower() for word in [
    ...
    def test_case_facts_template_structure(self):
        ...
        required_headings = [
            ...
            'Evidence', 'Arguments', 'Procedural', 'Jurisdiction', 
            'Law', 'Objectives'
        ]
        
        # At least most headings should be present (allowing for variations)
        found_headings = sum(1 for heading in required_headings 
                           if heading.lower() in case_facts.lower())
        
        assert found_headings >= 7, f"Case facts template missing essential headings. Found {found_headings}/10"
        ...
        def check_dict_values(d, path=""):
            for key, value in d.items():
                ...
                if isinstance(value, dict):
                    ...
                    pytest.fail(f"Template at {current_path} is not a string: {type(value)}")
