...
""")
    ...
    with patch.object(litassist.config.Config, '_find_config_file', return_value=str(config_path)):
...
def mock_external_apis():
    ...
    with patch('googleapiclient.discovery.build') as mock_google, \
         ...
         patch('pinecone.init') as mock_pinecone_init, \
         patch('pinecone.Index') as mock_pinecone_index, \
         patch('litassist.llm.LLMClientFactory.for_command') as mock_llm_factory:
        ...
        mock_openai_instance.embeddings.create.return_value = Mock(data=[Mock(embedding=[0.1]*1536)])
...
def test_all_commands_import_with_real_config(test_config_file):
    ...
    from litassist.commands import (  # noqa: F401
        lookup, digest, brainstorm, extractfacts,
        draft, strategy, verify, counselnotes,
        barbrief, caseplan
    )
...
def test_command_registration(test_config_file, mock_external_apis):
    ...
    expected_commands = {
        'barbrief', 'brainstorm', 'caseplan', 'counselnotes',
        'digest', 'draft', 'extractfacts', 'lookup',
        'strategy', 'verify'
...
def test_all_commands_show_help(test_config_file, mock_external_apis):
    ...
    all_commands = [
        'barbrief', 'brainstorm', 'caseplan', 'counselnotes',
        'digest', 'draft', 'extractfacts', 'lookup',
        'strategy', 'verify'  # 'test' is in cli.py directly
    ]
    ...
    for cmd in all_commands:
        ...
        assert "Options:" in result.output or "Usage:" in result.output, f"{cmd} missing Options/Usage section"
...
def test_file_processing_commands(mock_pinecone, mock_embeddings, test_config_file, mock_external_apis, tmp_path):
    """
    ...
    assert result.exit_code in [0, 1], f"extractfacts failed unexpectedly: {result.output}"
    assert "AttributeError" not in result.output
    ...
    assert result.exit_code in [0, 1], f"counselnotes failed unexpectedly: {result.output}"
    assert "AttributeError" not in result.output
...
""")
        ...
        result = runner.invoke(cli, ['barbrief', 'case_facts.txt', '--hearing-type', 'trial'])
...
def test_question_based_commands(mock_fetch, test_config_file, mock_external_apis):
    ...
    with patch('litassist.helpers.retriever.Retriever'), \
         patch('litassist.utils.text_processing.create_embeddings'):
        # Create a test file
        with runner.isolated_filesystem():
...
""")
        ...
        result = runner.invoke(cli, ['strategy', 'case_facts.txt', '--outcome', 'settlement'])
...
def test_api_test_command(mock_scraping, mock_validate, test_config_file, mock_external_apis):
...
def test_refactored_utils_imports(test_config_file):
    ...
    from litassist.utils.text_processing import chunk_text, create_embeddings  # noqa: F401
