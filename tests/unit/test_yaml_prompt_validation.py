...
class TestYAMLPromptValidation:
    ...
    def test_all_placeholders_are_valid(self, prompts_dir):
        ...
        valid_placeholders = {
            ...
            'facts_content', 'outcome', 'legal_issues', 'facts', 'side', 'area',
            'research', 'research_context', 'orthodox_strategies', 'unorthodox_strategies',
            'strategy_count', 'selected_count', 'remaining_strategies_list',
            'strategies_list', 'format_instructions', 'content', 'chunk_template',
            'chunk', 'digest_prompt', 'question', 'links', 'context', 'prompt',
            
            # Error and status placeholders
            'operation', 'error', 'command_name', 'chunk_idx', 'path',
            'issue', 'applicable_law', 'application', 'conclusion', 'confidence',
            'sources_text', 'command', 'timestamp', 'sources', 'option_num',
            'strategy_content', 'option_number', 'existing_count',
            'existing_titles', 'base_prompt', 'feedback', 'citation_instructions',
            
            # Document and file placeholders
            'recommended_strategy', 'chunk_num', 'total_chunks', 'name', 'place',
            'court_name', 'file_number', 'plaintiff_name', 'defendant_name',
            'size', 'max_size', 'word_count', 'file_type', 'count', 'key', 'value',
            'log_type', 'config_key', 'model', 'error_type', 'citation', 'reason',
            
            # Processing placeholders
            'context_instruction', 'documents', 'chunk_analyses', 'document_type',
            'user_request', 'all_facts', 'hearing_date', 'hearing_type', 'hearing_time',
            'jurisdiction', 'registry', 'judge', 'judicial_officer', 'party_capacity',
            
            # System feedback placeholders
            'found', 'expected_area', 'doc_path', 'length', 'line_preview', 'line',
            'link', 'index', 'extract_type', 'analysis_type', 'missing_list',
            'strategy_numbers', 'strategy_type', 'max_attempts', 'title', 'output_file',
            'function_name', 'duration', 'start_timestamp', 'end_timestamp',
            'attempt', 'retry_attempt', 'max_retries', 'item', 'strategy_num',
            'issue_count', 'categorized_issues',
            
            # Configuration placeholders
            'litassist_capabilities', 'max_tokens', 'temperature', 'reasoning_effort',
            'service', 'budget', 'instructions', 'request',
            
            # Document-specific placeholders
            'applicant_name', 'respondent_name', 'deponent_name', 'deponent_full_name',
            'deponent_occupation', 'deponent_address', 'deponent_role', 'annexure_mark',
            'witness_full_name', 'witness_address', 'witness_occupation',
        ...
        for yaml_file in yaml_files:
            ...
            for placeholder in placeholders:
                if placeholder not in valid_placeholders:
                    ...
                    possible_typos = [
                        valid for valid in valid_placeholders 
        ...
        for prompt_key in critical_prompts:
            try:
                ...
                if not content or len(content.strip()) < 10:
                    errors.append(f"{prompt_key}: Empty or too short (less than 10 chars)")
            except KeyError:
