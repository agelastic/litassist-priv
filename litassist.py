...
def timed(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logging.debug(f"Function {func.__name__} execution time: {duration:.3f} seconds")
            ...
            end_time = time.time()
            duration = end_time - start_time
            logging.debug(f"Function {func.__name__} failed after {duration:.3f} seconds")
            raise e
...
def save_log(tag: str, payload: dict):
    ...
    log_format = ctx.obj.get("log_format", "markdown") if ctx and ctx.obj else "markdown"
...
class Retriever:
    ...
    def __init__(self, index, use_mmr: bool = True, diversity_level: float = 0.3):
        ...
        self.diversity_level = max(0.0, min(1.0, diversity_level))  # Clamp between 0 and 1

    @timed
    def retrieve(self, query_emb: list[float], top_k: int = 5, diversity_level: float = None) -> list[str]:
        """
        ...
        if getattr(self, "use_mmr", False):
            ...
            actual_diversity = diversity_level if diversity_level is not None else self.diversity_level
            actual_diversity = max(0.0, min(1.0, actual_diversity))  # Clamp between 0 and 1
...
def lookup(question, mode, verify):
    ...
    save_log("lookup", {
        ...
        "inputs": {"question": question, "links": "\n".join(links), "prompt": prompt},
        "response": content,
        "usage": usage
    })
    click.echo(content)
...
def digest(file, mode, verify):
    ...
    for idx, chunk in enumerate(click.progressbar(chunks, label="Processing chunks"), start=1):
        prompt = (
        ...
        if verify:
            try:
                ...
                raise click.ClickException(f"Self-verification error in digest chunk {idx}: {e}")
                
        # Save audit log for this chunk
        save_log(f"digest_{mode}", {
            "file": file,
            "chunk": idx,
            "verify": verify,
            "response": content,
...
def extractfacts(file, verify):
    ...
    if verify:
        try:
            ...
            raise click.ClickException(f"Self-verification error during fact extraction: {e}")
    
    with open("case_facts.txt", "w", encoding="utf-8") as f:
        f.write(combined)

    # Audit log
    save_log("extractfacts", {
...
@click.option("--verify", is_flag=True, help="Enable self-critique pass")
@click.option("--diversity", type=float, help="Control diversity of search results (0.0-1.0)", default=None)
