...
def fetch_citation_context(citations: List[str], max_citations: int = 3) -> Dict[str, str]:
    """
    ...
    for citation in citations[:max_citations]:  # Limit for token management
        ...
        if not url and cse_comprehensive:
            try:
                ...
                save_log("cove_comprehensive_search_error", {
                    "citation": citation,
                    "error": str(e)
                })
        ...
        if url:
            try:
                ...
                if content:
                    ...
                    save_log("cove_document_fetched", {
                        "citation": citation,
                        "url": url,
                        "size_chars": len(context[citation])
                    })
            except Exception as e:
                save_log("cove_fetch_error", {
                    "citation": citation,
                    "url": url,
                    "error": str(e)
                })
        else:
            save_log("cove_no_url_found", {
                "citation": citation,
...
def _extract_section(text: str, citation: str) -> Optional[str]:
    ...
    if not sections:
        ...
        simple_match = re.search(rf'\b{section_num}\b.*?(?:misleading|deceptive|conduct)', 
