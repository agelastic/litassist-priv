...
def draft(ctx, documents, query, cove, diversity, output):
    ...
    if cove:
        try:
            ...
            from litassist.utils import warning_message, success_message
            ...
            if not cove_results['cove']['passed']:
                ...
                # Add note about regeneration to top of document
                cove_note = "=== DOCUMENT REGENERATED BY CHAIN OF VERIFICATION ===\n"
                cove_note += "This document was automatically corrected to fix the following issues:\n"
                cove_note += cove_results['cove']['issues'] + "\n"
                cove_note += "=" * 53 + "\n\n"
                content = cove_note + content
            else:
