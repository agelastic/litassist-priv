    ...
    if citations:
        try:
            ...
            citation_file = os.path.join(
                OUTPUT_DIR,
                f"verify_{os.path.basename(base_name)}_citations_{timestamp}.txt",
            )
        ...
        try:
            ...
            if reasoning_response:
                reasoning_file = os.path.join(
                    OUTPUT_DIR,
                    f"verify_{os.path.basename(base_name)}_reasoning_{timestamp}.txt",
                )
                with open(reasoning_file, "w", encoding="utf-8") as f:
                    f.write("# Legal Reasoning Analysis\n\n")
                    f.write(f"**Generated by**: {model_name}\n")
                    f.write(f"**Timestamp**: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                    f.write("**Command**: verify --reasoning\n\n")
                    f.write(f"---\n\n{reasoning_response}")
    ...
    if soundness:
        try:
            ...
            soundness_file = os.path.join(
                OUTPUT_DIR,
                f"verify_{os.path.basename(base_name)}_soundness_{timestamp}.txt",
            )
