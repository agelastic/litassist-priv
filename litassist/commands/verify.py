    ...
    if citations:
        try:
            ...
            citation_file = save_command_output(
                ...
                metadata={"Type": "Citation Verification", "File": file}
            )
    ...
    if reasoning:
        try:
            ...
            if reasoning_response:
                # Add header to the reasoning response
                full_reasoning_content = (
                    f"# Legal Reasoning Analysis\n\n"
                    f"**Generated by**: {model_name}\n"
                    f"**Timestamp**: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                    f"**Command**: verify --reasoning\n\n"
                    f"---\n\n{reasoning_response}"
                )
                reasoning_file = save_command_output(
                    ...
                    full_reasoning_content,
                    "" if output else os.path.basename(base_name),
                    metadata={"Type": "Reasoning Trace", "File": file, "Model": model_name}
                )
    ...
    if soundness:
        try:
            ...
            soundness_file = save_command_output(
                ...
                soundness_report,
                "" if output else os.path.basename(base_name),
                metadata={"Type": "Legal Soundness", "File": file, "Model": soundness_model}
            )
